##################################################################################################################################
## [B] BASIC INPUTS FOR ESTIMATIONS - CREATION OF RAS-EXPANSION OF REGIONAL IOTs SERIES
##################################################################################################################################

if(series_ras_expand == TRUE){
  if(length(files_path_figaro[files_path_figaro=="city_iots_ind_expand.rds"])==0){
    data_io_year <- data.frame()
    for(year_n in c(year_str_io:year_end_city)){
      p1  <- accounts_city %>% pivot_longer(!!sym(paste0(year_str_io)):!!sym(paste0(year_end_city)),names_to = "year",
      values_to = "value") %>% pivot_wider(names_from = "var",values_from = "value") %>% filter(year == year_n) %>% select(nace_code,P1)
      p2  <- accounts_city %>% pivot_longer(!!sym(paste0(year_str_io)):!!sym(paste0(year_end_city)),names_to = "year",
      values_to = "value") %>% pivot_wider(names_from = "var",values_from = "value") %>% filter(year == year_n) %>% select(nace_code,P2)
      b2a3g  <- accounts_city %>% pivot_longer(!!sym(paste0(year_str_io)):!!sym(paste0(year_end_city)),names_to = "year",
      values_to = "value") %>% pivot_wider(names_from = "var",values_from = "value") %>% filter(year == year_n) %>% select(nace_code,B2A3G)
      d29xd39  <- accounts_city %>% pivot_longer(!!sym(paste0(year_str_io)):!!sym(paste0(year_end_city)),names_to = "year",
      values_to = "value") %>% pivot_wider(names_from = "var",values_from = "value") %>% filter(year == year_n) %>% select(nace_code,D29XD39)
      d1 <- accounts_city %>% pivot_longer(!!sym(paste0(year_str_io)):!!sym(paste0(year_end_city)),names_to = "year",
      values_to = "value") %>% pivot_wider(names_from = "var",values_from = "value") %>% filter(year == year_n) %>% select(nace_code,D1)
      gdp <- accounts_city %>% pivot_longer(!!sym(paste0(year_str_io)):!!sym(paste0(year_end_city)),names_to = "year",
      values_to = "value") %>% pivot_wider(names_from = "var",values_from = "value") %>% filter(year == year_n) %>% select(nace_code,GDP)
      p2b <- accounts_city %>% pivot_longer(!!sym(paste0(year_str_io)):!!sym(paste0(year_end_city)),names_to = "year",
      values_to = "value") %>% filter(year == year_n) %>% pivot_wider(names_from = "var",values_from = "value") %>% 
      mutate(B1G = rowSums(across(c("D1","D29XD39","B2A3G")),na.rm=TRUE)) %>% mutate(P2B = P1-GDP) %>% mutate(P2C = P1-B1G) %>% 
      select(nace_code,P2B,P2C)
        
      #### [b] correct the regional table with municipal inputs
      year_ciot_n <- years[which.min(abs(years - year_n))]
      z_matrix    <- nio.mad.matrix(year_ciot_n,"",'Z',"D","city")
      im_matrix   <- nio.mad.matrix(year_ciot_n,"",'P7M',"D","city")
      t_matrix    <- nio.mad.matrix(year_ciot_n,"",'VMT',"D","city")
      zx_matrix   <- nio.mad.matrix(year_ciot_n,"",'ZT',"D","city") %>% colSums %>% as.data.frame() %>%
      ...
      ##### Temurshoev et al. (2013, p. 363): main vectors are row and column totals u_i and v_j
      ##### [==> the "v" is the result of correcting the distribution the new matrix colsums by the distribution of P2
      v <- p2 %>% left_join(colSums(z_matrix) %>% as.data.frame %>% rename("P2B"=".") %>% rownames_to_column("code"),
      ...
      ##### it is important to adjust the distribution and, then, to correct the levels to match the "v"
      u <- rowSums(nio.mad.matrix(year_ciot_n,"",'Z',"D","city")) %>% as.data.frame %>% rename("P2"=".") %>% rownames_to_column("code") %>%
      left_join(nio.mad.matrix(year_ciot_n,"",'P1',"D","city")%>% as.data.frame %>% rownames_to_column("code"),by=c("code")) %>%
      mutate(v_ratio = (P2/P1)) %>% left_join(p1,by=c("code"="nace_code")) %>% mutate(v = v_ratio * P1.y) %>%
      #### [ATTENTION] level correction so that vector totals match using P2 from municipal accounts as benchmark
      bind_cols("u" = v) %>% mutate_at(c("v"),~v*(sum(u)/sum(v))) %>% mutate(sum(u),sum(v)) %>% select(v) %>%
      mutate_all(~if_else(is.na(.),0,.)) %>% unlist %>% as.vector
      ##### [==> apply GRAS algorithm
      z_matrix_ras <- gras_update(u = u, v = v, X0 = as.matrix(z_matrix),epsilon=0.00001) %>% 
      ...
      cat(paste0("\n Output: year ",year_n," using ",year_ciot_n," RIOT \n"))
    }
    saveRDS(data_io_year,file.path(getwd(),"datasets","city_iots_ind_expand.rds"))
  } else{
    city_iots_industry <- readRDS(file.path(getwd(),"datasets","city_iots_ind_expand.rds"))
  }
  
  if(length(files_path_figaro[files_path_figaro=="city_mrio_industry_final_extended.rds"])==0){
  ...
  for(year_n in seq(year_str_io,year_end_city)){
    ...
    import_vec <- nio.mad.matrix(year_ciot_n,"",'M',"D","city") %>% rownames_to_column("code") %>% as_tibble %>% filter(code%in%c(as.vector(unlist(code_iots_city %>%
    ...
    pivot_longer(`A`:`T`,names_to = "code",values_to = "P7") %>% left_join(nio.mad.matrix(year_ciot_n,"",'M',"D","city") %>% rownames_to_column("code") %>% 
    ...
    cat("the RoN and RoW import matrices match the imported domestic (local) totals \n") } else {
    ...
    fd_exports <- nio.city.matrix('ES',year_ciot_n,"",'P6',"industry") %>% rownames_to_column("code") %>% 
    left_join(nio.mad.matrix(year_ciot_n,"",'P6',"D","city") %>% rownames_to_column("code"),by=c("code")) %>%
    ...
    cat("the RoN and RoW export matrices match the exported domestic (local) totals \n") } else {
    cat("the RoN and RoW export matrices DO NOT the exported domestic (local) totals \n")}
      ...
      nio.city.matrix('ES',year_ciot_n,"",'P6',"industry") %>% rownames_to_column("code") %>% left_join(
      nio.mad.matrix(year_ciot_n,"",'P6',"D","city") %>% rownames_to_column("code"),by=c("code")) %>%
      set_names(c("code","nation","region")) %>% mutate_at(c("nation","region"),~./sum(.,na.rm=TRUE)) %>%
    ...
    cat("the new and original MRIOs match \n") } else {cat("the new and original MRIOs DO NOT match \n")}
      ...
      mutate(P1 = rowSums(across(`AR_A`:`ZA_P5M`),na.rm=TRUE)) %>% select(code,P1) %>% separate(code,c("ref_area","code"),extra = "merge") %>% 
      filter(ref_area=="ES") %>% rename("ES"="P1") %>% left_join(accounts_city %>% pivot_longer(!!sym(paste0(year_str_io)):!!sym(paste0(year_end_city)),names_to = "year",values_to = "value") %>% 
      ...
      rownames_to_column("code") %>% mutate(ref_area = cou_n))}
        ...
        bind_rows(accounts_city %>% pivot_longer(!!sym(paste0(year_str_io)):!!sym(paste0(year_end_city)),names_to = "year",values_to = "value") %>% filter(var=="GDP") %>%
      ...
      c("counter_area","counter_code"),extra = "merge") %>% pivot_wider(names_from = "counter_area",values_from = "obs_value") %>%
        ...
        left_join(accounts_city %>% pivot_longer(!!sym(paste0(year_str_io)):!!sym(paste0(year_end_city)),names_to = "year",values_to = "value") %>% select(-code) %>% 
      ...
      select(`AR_A`:`ZA_T`)),15)==1){cat("the new and original MRIOs GVA vectos match \n")}else{
    ...
    cat(paste0("\n Output: FIGARO integration of city IOT for year ",year_n," complete \n"))
    }
     #### save the aggregated dataset
    saveRDS(data_io_mrio,file.path(getwd(),"datasets","city_mrio_industry_final_extended.rds"))
  } else {
    city_mrio_industry_final <- readRDS(file.path(getwd(),"datasets","city_mrio_industry_final_extended.rds"))
