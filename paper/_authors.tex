...
$it.name.literal$$if(it.orcid)$~\orcidlink{$it.orcid$}$endif$\\$for(it.affiliations/first)$$it.name$$endfor$\\$if(it.email)$Contact at: \href{mailto:$it.email$}{$it.email$}$endif$\\$if(it.draft)$$it.draft$$endif$